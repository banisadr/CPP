    ...
    ui(new Ui::MainWindow)
{
  ...
  curr_user = NULL;
    ...
    ui->pwd_edit_check->setEchoMode(QLineEdit :: Password);
}
...
void MainWindow::on_stackedWidget_currentChanged(int arg1) //When Profile Widget Becomes Active
{
  if (curr_user != NULL){
    ...
    ui->first->setText(curr_user->fname);
    ui->last->setText(curr_user->lname);
...
void MainWindow::on_pushButton_3_clicked() //Search Dialog Used
{
    ...
    std::vector<User> arr = User::getSearchResults(search);

    ui->tableWidget->setColumnCount(6);
    ui->tableWidget->setRowCount(arr.size());
    QStringList header = (QStringList() << "First Name"
                          << "Last Name" << "Phone"
                          << "Position" << "Email" << "Location");
    ui->tableWidget->setHorizontalHeaderLabels(header);

    for(int i = 0; i < arr.size(); i++)
    {
        User u = arr[i];
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(u.fname));
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(u.lname));
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(u.phone));
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(u.position));
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(u.email));
        ui->tableWidget->setItem(i,5,new QTableWidgetItem(u.location));
    }
...
void MainWindow::on_pushButton_2_clicked() //Click Create Profile
{
    ...
    ui->first_edit->setText("");
    ui->last_edit->setText("");
    ui->phone_edit->setText("");
    ui->position_edit->setText("");
...
void MainWindow::on_save_clicked() //Click Save Profile Edits
{
    ...
    QString first = ui ->first_edit->text();
    QString last = ui ->last_edit->text();
    ...
    curr_user->fname = first;
    curr_user->lname = last;
...
void MainWindow::on_edit_clicked() //Clicked Edit From Profile
{
  ...
  if (curr_user != NULL){
    ...
    ui->first_edit->setText(curr_user->fname);
    ui->last_edit->setText(curr_user->lname);
